package continuous.alfred.study.conurrenttool.copyonwritelistmisuse;
...
public class CopyOnWriteListMisuseController {
    ...
    public Map testWrite() {
        ...
        IntStream.rangeClosed(1, loopCount)
                .parallel()
                .forEach(__ -> copyOnWriteArrayList.add(
        ...
        IntStream.rangeClosed(1, loopCount)
                .parallel()
                .forEach(__ -> synchronizedList.add(
                        ThreadLocalRandom.current().nextInt(loopCount)));
        stopWatch.stop();
    ...
    public Map testRead() {
        ...
        IntStream.rangeClosed(1, loopCount)
                .parallel()
                .forEach(__ -> copyOnWriteArrayList.get(
        ...
        IntStream.range(0, loopCount)
                .parallel()
                .forEach(__ -> synchronizedList.get(
                        ThreadLocalRandom.current().nextInt(count)));
        stopWatch.stop();
